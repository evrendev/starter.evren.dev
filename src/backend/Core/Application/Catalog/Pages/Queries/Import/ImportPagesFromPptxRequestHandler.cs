...
public class ImportPagesFromPptxRequest : IRequest<Guid>
{
...
public class ImportPagesFromPptxRequestValidator : CustomValidator<ImportPagesFromPptxRequest>
{
    public ImportPagesFromPptxRequestValidator(IReadRepository<Course> courseRepo,
        IStringLocalizer<ImportPagesFromPptxRequestValidator> localizer)
    {
        ...
        RuleFor(p => p.File)
            ...
            .Must(f => f.Length is > 0 and <= ImportPagesFromPptxRequestHandler.MaxFileSizeBytes)
                .WithMessage(
                    $"File size must be between 1 byte and {ImportPagesFromPptxRequestHandler.MaxFileSizeBytes / (1024 * 1024)}MB.");
    }
}
...
public class ImportPagesFromPptxRequestHandler(
    IReadRepository<Course> courseRepository,
    IRepository<ImportJob> importJobRepository,
    IFileStorageService fileStorageService,
    IJobService jobService,
    ICurrentUser currentUser,
    ILogger<ImportPagesFromPptxRequestHandler> logger) : IRequestHandler<ImportPagesFromPptxRequest, Guid>
{
    public const long MaxFileSizeBytes = 150 * 1024 * 1024;

    public async Task<Guid> Handle(ImportPagesFromPptxRequest request, CancellationToken cancellationToken)
    {
        ...
        jobService.Enqueue<IImportPagesFromPptxJob>(
